...
DICT_PATH = "C:/Users/Administrator/Downloads/OxfordAdvanced/牛津高阶英汉双解词典(第9版)_V3.1.2版.mdx"


@register(["本地词典-牛津高阶9JSON提取", "牛津高阶9JSON提取"])
class oalecd9_mdx(MdxService):
    ...
    def meanings(self):
        ...
        if definitions:
            for i, definition in enumerate(definitions):
                def_str = (
                    def_str + str(i + 1) + ". " + definition.get_text() + "<br><br>"
                )
        return def_str
    ...
    def idioms(self):
        ...
        for idiom in idioms:
            ...
            result.append(
                {
                    "usage": entry.get_text(),
                    "defs": self._get_definition_and_examples(idiom),
                }
            )
    ...
    def phrase_verbs(self):
        ...
        for phrv in soup.find_all("pv-g"):
            ...
            result.append(
                {
                    "usage": " | ".join(usages),
                    "defs": self._get_definition_and_examples(phrv),
                }
            )
        if not result:
            return ""
