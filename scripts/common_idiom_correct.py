...
raw_mapping = get_raw_mapping_manually([c for c,_ in mapping])

with open(html_dir / 'correct_mapping_html.json', mode='w', encoding='utf-8') as f:
    json.dump(list(html_mapping), f)
with open(html_dir / 'correct_mapping_entry.json', mode='w', encoding='utf-8') as f:
    json.dump(list(entry_mapping), f)
with open(html_dir / 'correct_mapping_unknown.json', mode='w', encoding='utf-8') as f:
    json.dump(list(raw_mapping), f)
with open(html_dir / 'correct_mapping_manual.json', mode='w', encoding='utf-8') as f:
    json.dump({
        "manual": manual_mapping,
        "duplicate_kanji": duplicate_kanji_map
    }, f)
