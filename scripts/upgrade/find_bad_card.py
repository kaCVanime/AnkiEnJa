...
def correct():
    bad = [("chimney sweep", "sweep"), ("bell pepper", "pepper"), ("blowlamp", "torch"), ("courtyard", "court"),
           ("talent scout", "scout"), ("faze", "phase"), ("mom", "mum"), ("snarl-up", "snarl"),
           ("bell-ringer", "ringer"), ("newsflash", "flash"),
           ("cogwheel", "cog"), ("wack", "whack"), ("steeplechase", "chase"), ("rocking chair", "rocker"),
           ("flippant", "flip"), ("golf course", "course"), ("feint", "faint"), ("oil well", "well"),
           ("marijuana", "pot"), ("sett", "set"), ("worktop", "counter"), ("bhang", "bang"),
           ("mother-of-pearl", "pearl"), ("flagstone", "stone"), ("indium", "in")]
    bad = [("indium", "in")]
    ...
    for weird, origin in tqdm(bad):
        nids = anki_invoke("findNotes", query=f'(deck:KEXP::Read OR deck:KEXP::Write) "term:{weird}" -usage:')
        for nid in nids:
            note = get_note_fields(nid)
            normal_id = note["id"].replace(weird.replace(" ", "_"), origin.replace(" ", "_"))
...
def run():
    ...
    for nid in tqdm(read_nids):
        note = get_note_fields(nid)
        if note["usage"] and "~" not in note["usage"] and "+" not in note["usage"] and not term_in_usage(
                note["term"],
                note["usage"]
        ):
